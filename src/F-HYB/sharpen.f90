...
!  a hybrid of MPI processes and OpenMP threads, with replicated data.
!  The master process reads in the fuzzy 
...
!$omp end parallel
  ...
  if (rank == 0) then
     ...
     write(*,*)
     write(*,fmt='(" Image sharpening code running on", I3," process(es) with", I3, " thread(s) per process")') size, nthreads
